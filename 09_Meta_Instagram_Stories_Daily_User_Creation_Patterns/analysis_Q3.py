...
# Identify users with at least one day > 10 stories
high_volume_users = daily_totals[daily_totals['story_count'] > 10]['user_id'].unique()

# Calculate percentage
total_users = daily_totals['user_id'].nunique()
percentage = (len(high_volume_users) / total_users) * 100

# Output results
print("High Volume Story Posters Analysis")
print("="*50)
print(f"Total unique users: {total_users}")
print(f"Users with >10 stories on at least one day: {len(high_volume_users)}")
print(f"Percentage of high-volume users: {percentage:.1f}%")
print("\nUsers with high-volume posting days:")
print(pd.Series(high_volume_users).to_string(index=False))